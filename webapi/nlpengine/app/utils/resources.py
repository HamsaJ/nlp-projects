...
RE_USER_HANDLE: Pattern = re.compile(
    r"(?:^|(?<![\w@.]))@\w+", flags=re.UNICODE | re.IGNORECASE
)

RE_HASHTAG: Pattern = re.compile(
    r"(?:^|(?<![\w#＃.]))(#|＃)(?!\d)\w+", flags=re.UNICODE | re.IGNORECASE
)
...
if sys.maxunicode < 0x10FFFF:
    RE_EMOJI = re.compile(
        r"[\u2600-\u26FF\u2700-\u27BF]", flags=re.UNICODE | re.IGNORECASE
    )
else:
    RE_EMOJI = re.compile(
...
RE_HYPHENATED_WORD: Pattern = re.compile(
    r"(\w{2,}(?<!\d))\-\s+((?!\d)\w{2,})", flags=re.UNICODE | re.IGNORECASE
)
